...
async def async_request_openai_completions(
    ...
    async with aiohttp.ClientSession(timeout=AIOHTTP_TIMEOUT) as session:
        ...
        try:
            ...
            print('len request_func_input.prompt', len(request_func_input.prompt))
            exc_info = sys.exc_info()
            output.error = "".join(traceback.format_exception(*exc_info))
...
def sample_random_requests(
    ...
    if True:
        ...
        for i in range(num_prompts):
            ...
            prompt = dataset[i][0]
            prompt_token_ids = tokenizer(prompt).input_ids
            encode_ids=tokenizer.encode(prompt)
            assert len(prompt_token_ids)==len(encode_ids)
            ...
            encode_len=len(tokenizer.encode(prompt))
            # print('encode_len', encode_len)
            # print('len input_ids', len(input_ids))
            # print('input_lens[i]', input_lens[i])

            input_requests.append((prompt, int(input_lens[i]), int(output_lens[i])))
            if 'pwd'in prompt:
                print('encode_len 111111111111 ', encode_len)
                print('input_lens[i] 11111111111111', input_lens[i])
                # print(prompt)
                # print(input_lens[i])
                # print(input_ids)
                print('len', len(input_ids))
                
                ids_encode=tokenizer.encode(prompt)
                # print('ids_encode', ids_encode)
                print('len', len(ids_encode))
...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            print(f"Request failed: {outputs[i].error}")
...
def fire(args: argparse.Namespace):
    ...
    if args.model is None:
        try:
            ...
            print(f"Model not specified. Using the first model: {args.model}")
        except Exception as e:
